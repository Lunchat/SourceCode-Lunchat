        ...
        <div class="bg-white mx-auto p-50 buat-text" data-aos="fade-down" data-aos-duration="200" style="width: 100%; border-radius:5px;">
            <div class="row" style="color: black; font-family: 'poppins';">
                <div class="col-md-12 mt-1 text-center">
                    <h1 class="display-50" data-aos="fade-down" data-aos-duration="200">SPEAKING CLASS
                    </h1>
                    <hr width="80%">
    ...
    @endif
        ...
        <div class="row mt-4">
            @foreach( $speaking as $s)
                <div class="col-md-6 mb-4" data-aos="fade-right" data-aos-duration="1200">
                    <div class="card material w-150 border-0">
                        ...
                        <div class="card-body p-5">
                            ...
                            <p class=" card-text">
                                Topik Pembelajaran : {{ $s->topik }}
                            </p>
                            <p class=" card-text">
                                Waktu : {{ $s->tanggal }}
                            </p>
                            <p class=" card-text">
                                Mulai : {{ $s->jam_mulai }} s/d {{ $s->jam_selesai }}
                            </p>
                            @php
                            $peserta = DB::table('speaking_joins')
                                        ->where('speaking_id', $s->id)
                                        ->count();
                            $kuota = $s->kuota - $peserta;
                            @endphp
                            <p class=" card-text">
                                Kuota : {{ $kuota }}
                            </p>
                ...
                @php 
                $exist = DB::table('speaking_joins')
                        ->where('student_id', Auth()->user()->id)
                        ->where('speaking_id', $s->id)
                        ->exists();
                @endphp
                @if($exist)
                <a href="/student/room_speaking/{{ $s->id }}" class="btn btn-warning">Detail</a>  
                @else
                    @if ($kuota != 0)
                        <form method="post" enctype="multipart/form-data" action="https://api.whatsapp.com/send?phone=6281224669308&text=Halo%20Admin%20Saya%20Mau%20Daftar%20Speaking%20Class">
                            {{ csrf_field() }} <!-- Tambah ini (Agar tidak eror 419 | Page Expired) -->
                            <input type="hidden" name="speaking_id" value="{{ $s->id }}">
