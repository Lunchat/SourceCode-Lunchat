        ...
        <div class="bg-white mx-auto p-50 buat-text" data-aos="fade-down" data-aos-duration="200" style="width: 100%; border-radius:5px;">
            <div class="row" style="color: black; font-family: 'poppins';">
                <div class="col-md-12 mt-1 text-center">
                    <h1 class="display-50" data-aos="fade-down" data-aos-duration="200">WRITING CLASS
                    </h1>
                    <hr width="80%">
    ...
    @endif
        ...
        <div class="row mt-4">
            @foreach( $writing as $w)
                <div class="col-md-6 mb-4" data-aos="fade-right" data-aos-duration="1200">
                    <div class="card material w-150 border-0">
                        ...
                        <div class="card-body p-5">
                            ...
                            @php
                            $peserta = DB::table('writing_joins')
                                        ->where('writing_id', $w->id)
                                        ->count();
                            $kuota = $w->kuota - $peserta;
                            @endphp
                            <p class=" card-text">
                                Kuota : {{ $kuota }}
                            </p>
                ...
                @php 
                $exist = DB::table('writing_joins')
                        ->where('student_id', Auth()->user()->id)
                        ->where('writing_id', $w->id)
                        ->exists();
                @endphp
                @if($exist)
                <a href="/student/room_writing/{{ $w->id }}" class="btn btn-warning">Detail</a>  
                @else
                    @if ($kuota != 0)
                        <form method="post" enctype="multipart/form-data" action="https://api.whatsapp.com/send?phone=6281224669308&text=Halo%20Admin%20Saya%20Mau%20Daftar%20Writing%20Class">
