            ...
            <div class="col-md-12 w-150 mb-4">
                <div class="card material border-0">
                    <div class="card-body p-5">
                        <h2 class="card-title">{{ $grammar->name }}</h2>
...
<div class="button">
  <ul class="right">
    <a href='https://calendar.google.com/calendar/r/eventedit?text={{ $grammar->name }} - {{ $grammar->topik }}&amp;dates=<?= date_format(date_create($grammar->tanggal), "Ymd") ?>T<?= date_format(date_create($grammar->jam_mulai), "Hi00") ?>/<?= date_format(date_create($grammar->tanggal), "Ymd") ?>T<?= date_format(date_create($grammar->jam_selesai), "Hi00") ?>&amp;ctz=Asia/Jakarta&amp;details=&amp;location=Location&amp;pli=1&amp;uid=1521339627addtocalendar&amp;sf=true&amp;output=xml' target='blank' class="btn btn-sm btn-info">Save the Date <i class="fa fa-calendar"></i></a>
  </ul>
</div>
                        
                        <div>
                            <ul class="list-group list-group-flush">
    <p><li class="list-group-item"><p style="color:black";><i class="fas fa-book"></i> Materi Pembelajaran : {{ $grammar->topik }}</p></li>
    <p><li class="list-group-item"><p style="color:black";><i class="far fa-calendar"></i>  Tanggal : {{ $grammar->tanggal }}</p></li>
    <p><li class="list-group-item"><p style="color:black";><i class="far fa-clock"></i>  Jam : {{ $grammar->jam_mulai }} - {{ $grammar->jam_selesai }}</p></li></p>
    <li class="list-group-item"><p style="color:black";><i class="fas fa-user"></i>  Kuota : {{ $grammar->kuota }}<p></li><p>
  </ul><div class="card-body">
    
    <a href="{{ url('grammar'.'/'.$grammar->file) }}" class="btn btn-sm btn-warning"><i class="fas fa-download"></i>  Materi</a>&nbsp;
    <a href="{{ $grammar->url }}" class="btn btn-sm btn-primary"><i class="fas fa-video"></i> Masuk</a>
  </div>
