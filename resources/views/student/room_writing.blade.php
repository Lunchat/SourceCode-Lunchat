            ...
            <div class="col-md-12 w-150 mb-4">
                <div class="card material border-0">
                    <div class="card-body p-5">
                        <h2 class="card-title">{{ $writing->name }}</h2>
...
<div class="button">
  <ul class="right">
    <a href='https://calendar.google.com/calendar/r/eventedit?text={{ $writing->name }} - {{ $writing->topik }}&amp;dates=<?= date_format(date_create($writing->tanggal), "Ymd") ?>T<?= date_format(date_create($writing->jam_mulai), "Hi00") ?>/<?= date_format(date_create($writing->tanggal), "Ymd") ?>T<?= date_format(date_create($writing->jam_selesai), "Hi00") ?>&amp;ctz=Asia/Jakarta&amp;details=&amp;location=Location&amp;pli=1&amp;uid=1521339627addtocalendar&amp;sf=true&amp;output=xml' target='blank' class="btn btn-sm btn-info">Save the Date <i class="fa fa-calendar"></i></a>
  </ul>
</div>
                        
                        <div>
                            <ul class="list-group list-group-flush">
    <p><li class="list-group-item"><p style="color:black";><i class="fas fa-book"></i> Materi Pembelajaran : {{ $writing->topik }}</p></li>
    <p><li class="list-group-item"><p style="color:black";><i class="far fa-calendar"></i>  Tanggal : {{ $writing->tanggal }}</p></li>
    <p><li class="list-group-item"><p style="color:black";><i class="far fa-clock"></i>  Jam : {{ $writing->jam_mulai }} - {{ $writing->jam_selesai }}</p></li></p>
    <li class="list-group-item"><p style="color:black";><i class="fas fa-user"></i>  Kuota : {{ $writing->kuota }}<p></li><p>
  </ul><div class="card-body">
    
    <a href="{{ url('writing'.'/'.$writing->file) }}" class="btn btn-sm btn-warning"><i class="fas fa-download"></i>  Materi</a>&nbsp;
    <a href="{{ $writing->url }}" class="btn btn-sm btn-primary"><i class="fas fa-video"></i> Masuk</a>
  </div>
