...
<div class="button">
  <ul class="right">
    <a href='https://calendar.google.com/calendar/r/eventedit?text={{ $material->name }} - {{ $material->topik }}&amp;dates=<?= date_format(date_create($material->tanggal), "Ymd") ?>T<?= date_format(date_create($material->jam_mulai), "Hi00") ?>/<?= date_format(date_create($material->tanggal), "Ymd") ?>T<?= date_format(date_create($material->jam_selesai), "Hi00") ?>&amp;ctz=Asia/Jakarta&amp;details=&amp;location=Location&amp;pli=1&amp;uid=1521339627addtocalendar&amp;sf=true&amp;output=xml' target='blank' class="btn btn-sm btn-info">Save the Date <i class="fa fa-calendar"></i></a>
  </ul>
</div>
                        
                        <div>
                            <ul class="list-group list-group-flush">
    <p><li class="list-group-item"><p style="color:black";><i class="fas fa-book"></i> Materi Pembelajaran : {{ $material->topik }}</p></li>
    <p><li class="list-group-item"><p style="color:black";><i class="far fa-calendar"></i>  Tanggal : {{ $material->tanggal }}</p></li>
    <p><li class="list-group-item"><p style="color:black";><i class="far fa-clock"></i>  Jam : {{ $material->jam_mulai }} - {{ $material->jam_selesai }}</p></li></p>
    <li class="list-group-item"><p style="color:black";><i class="fas fa-user"></i>  Kuota : {{ $material->kuota }}<p></li><p>
  </ul><div class="card-body">
    
    <a href="{{ url('sharing'.'/'.$material->file) }}" class="btn btn-sm btn-warning"><i class="fas fa-download"></i>  Materi</a>&nbsp;
    <a href="{{ $material->url }}" class="btn btn-sm btn-primary"><i class="fas fa-video"></i> Masuk</a>
  </div>
